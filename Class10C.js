...
const Class10C=()=>{
    ...
    let userclasscopy = (classes.filter(u => u.teaches.includes("10C") || u.count === 1));
    let classteacher = userclasscopy.find(u => u.classteacher === "10C");
    ...
    useEffect(() => {
        axios.get("https://root-gold-cannon.glitch.me/api/teach")
          .then(response =>{
            ...
            const data=response.data.slice(432,480);
            setUser(data);
        })
          .catch(error => console.error("Error fetching users:", error));
      }, []);
    return(
        <div>
            <h2 style={{color:"darkblue",paddingTop:"3%"}}>Timetable of Class 10C</h2>
            ...
            <div style={{paddingLeft:"10%",paddingRight:"10%",display:"flex",justifyContent:"space-between"}}>
                <div>
                    <button onClick={()=>window.location.href="/10B"} style={{width:"150px"}} className="btn btn-primary">PREV(10B) </button>
                </div>
                ...
                <div>
                    <button onClick={()=>window.location.href="/10D"} style={{width:"150px"}} className="btn btn-primary">NEXT(10D) </button>
                </div>
            </div>
        </div>
    )
}
export default Class10C;