...
const Class9G=()=>{
    ...
    let userclasscopy = (classes.filter(u => u.teaches.includes("9G") || u.count === 1));
    let classteacher = userclasscopy.find(u => u.classteacher === "9G");
    ...
    useEffect(() => {
        axios.get("https://root-gold-cannon.glitch.me/api/teach")
          .then(response =>{
            ...
            const data=response.data.slice(288,336);
            setUser(data);
        })
          .catch(error => console.error("Error fetching users:", error));
      }, []);
    return(
        <div>
            <h2 style={{color:"darkblue",paddingTop:"3%"}}>Timetable of Class 9G</h2>
            ...
            <div style={{paddingLeft:"10%",paddingRight:"10%",display:"flex",justifyContent:"space-between"}}>
                <div>
                    <button onClick={()=>window.location.href="/9F"} style={{width:"150px"}} className="btn btn-primary">PREV(9F) </button>
                </div>
                ...
                <div>
                    <button onClick={()=>window.location.href="/10A"} style={{width:"150px"}} className="btn btn-primary">NEXT(10A) </button>
                </div>
            </div>
        </div>
    )
}
export default Class9G;