...
const Class10B=()=>{
    ...
    let userclasscopy = (classes.filter(u => u.teaches.includes("10B") || u.count === 1));
    let classteacher = userclasscopy.find(u => u.classteacher === "10B");
    ...
    useEffect(() => {
        axios.get("https://root-gold-cannon.glitch.me/api/teach")
          .then(response =>{
            ...
            const data=response.data.slice(384,432);
            setUser(data);
        })
          .catch(error => console.error("Error fetching users:", error));
      }, []);
    return(
        <div>
            <h2 style={{color:"darkblue",paddingTop:"3%"}}>Timetable of Class 10B</h2>
            ...
            <div style={{paddingLeft:"10%",paddingRight:"10%",display:"flex",justifyContent:"space-between"}}>
                <div>
                    <button onClick={()=>window.location.href="/10A"} style={{width:"150px"}} className="btn btn-primary">PREV(10A) </button>
                </div>
                ...
                <div>
                    <button onClick={()=>window.location.href="/10C"} style={{width:"150px"}} className="btn btn-primary">NEXT(10C) </button>
                </div>
            </div>
        </div>
    )
}
export default Class10B;