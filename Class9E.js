...
const Class9E=()=>{
    ...
    let userclasscopy = (classes.filter(u => u.teaches.includes("9E") || u.count === 1));
    let classteacher = userclasscopy.find(u => u.classteacher === "9E");
    ...
    useEffect(() => {
        axios.get("https://root-gold-cannon.glitch.me/api/teach")
          .then(response =>{
            ...
            const data=response.data.slice(192,240);
            setUser(data);
        })
          .catch(error => console.error("Error fetching users:", error));
      }, []);
    return(
        <div>
            <h2 style={{color:"darkblue",paddingTop:"3%"}}>Timetable of Class 9E</h2>
            ...
            <div style={{paddingLeft:"10%",paddingRight:"10%",display:"flex",justifyContent:"space-between"}}>
                <div>
                    <button onClick={()=>window.location.href="/9D"} style={{width:"150px"}} className="btn btn-primary">PREV(9D) </button>
                </div>
                ...
                <div>
                    <button onClick={()=>window.location.href="/9F"} style={{width:"150px"}} className="btn btn-primary">NEXT(9F) </button>
                </div>
            </div>
        </div>
    )
}
export default Class9E;