...
const Class9B=()=>{
    ...
    let userclasscopy = (classes.filter(u => u.teaches.includes("9B") || u.count === 1));
    let classteacher = userclasscopy.find(u => u.classteacher === "9B");
    ...
    useEffect(() => {
        axios.get("https://root-gold-cannon.glitch.me/api/teach")
          .then(response =>{
            ...
            const data=response.data.slice(48,96);
            setUser(data);
        })
          .catch(error => console.error("Error fetching users:", error));
      }, []);
    return(
        <div>
            <h2 style={{color:"darkblue",paddingTop:"3%"}}>Timetable of Class 9B</h2>
            ...
            <div style={{paddingLeft:"10%",paddingRight:"10%",display:"flex",justifyContent:"space-between"}}>
                <div>
                    <button onClick={()=>window.location.href="/9A"} style={{width:"150px"}} className="btn btn-primary">PREV(9A) </button>
                </div>
                ...
                <div>
                    <button onClick={()=>window.location.href="/9C"} style={{width:"150px"}} className="btn btn-primary">NEXT(9C) </button>
                </div>
            </div>
        </div>
    )
}
export default Class9B;