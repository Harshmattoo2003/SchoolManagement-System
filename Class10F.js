...
const Class10F=()=>{
    ...
    let userclasscopy = (classes.filter(u => u.teaches.includes("10F") || u.count === 1));
    let classteacher = userclasscopy.find(u => u.classteacher === "10F");
    ...
    useEffect(() => {
        axios.get("https://root-gold-cannon.glitch.me/api/teach")
          .then(response =>{
            ...
            const data=response.data.slice(576,624);
            setUser(data);
        })
          .catch(error => console.error("Error fetching users:", error));
      }, []);
    return(
        <div>
            <h2 style={{color:"darkblue",paddingTop:"3%"}}>Timetable of Class 10F</h2>
            ...
            <div style={{paddingLeft:"10%",paddingRight:"10%",display:"flex",justifyContent:"space-between"}}>
                <div>
                    <button onClick={()=>window.location.href="/10E"} style={{width:"150px"}} className="btn btn-primary">PREV(10E) </button>
                </div>
                ...
                <div>
                    <button onClick={()=>window.location.href="/10G"} style={{width:"150px"}} className="btn btn-primary">NEXT(10G) </button>
                </div>
            </div>
        </div>
    )
}
export default Class10F;