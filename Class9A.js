...
const Class9A=()=>{
    ...
    let userclasscopy = (classes.filter(u => u.teaches.includes("9A") || u.count === 1));
    let classteacher = userclasscopy.find(u => u.classteacher === "9A");
    ...
    useEffect(() => {
        axios.get("https://root-gold-cannon.glitch.me/api/teach")
          .then(response =>{
            ...
            const data=response.data.slice(0,48);
            setUser(data);
        })
          .catch(error => console.error("Error fetching users:", error));
      }, []);
    return(
        <div>
            <h2 style={{color:"darkblue",paddingTop:"3%"}}>Timetable of Class 9A</h2>
            ...
            <div style={{paddingLeft:"10%",paddingRight:"10%",display:"flex",justifyContent:"space-between"}}>
                <div>
                    <button onClick={()=>window.location.href="/9A"} style={{width:"150px"}} className="btn btn-primary">PREV </button>
                </div>
                ...
                <div>
                    <button onClick={()=>window.location.href="/9B"} style={{width:"150px"}} className="btn btn-primary">NEXT(9B) </button>
                </div>
            </div><hr/>
        </div>
    )
}
export default Class9A;