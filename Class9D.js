...
const Class9D=()=>{
    ...
    let userclasscopy = (classes.filter(u => u.teaches.includes("9D") || u.count === 1));
    let classteacher = userclasscopy.find(u => u.classteacher === "9D");
    ...
    useEffect(() => {
        axios.get("https://root-gold-cannon.glitch.me/api/teach")
          .then(response =>{
            ...
            const data=response.data.slice(144,192);
            setUser(data);
        })
          .catch(error => console.error("Error fetching users:", error));
      }, []);
    return(
        <div>
            <h2 style={{color:"darkblue",paddingTop:"3%"}}>Timetable of Class 9D</h2>
            ...
            <div style={{paddingLeft:"10%",paddingRight:"10%",display:"flex",justifyContent:"space-between"}}>
                <div>
                    <button onClick={()=>window.location.href="/9C"} style={{width:"150px"}} className="btn btn-primary">PREV(9C) </button>
                </div>
                ...
                <div>
                    <button onClick={()=>window.location.href="/9E"} style={{width:"150px"}} className="btn btn-primary">NEXT(9E) </button>
                </div>
            </div>
        </div>
    )
}
export default Class9D;